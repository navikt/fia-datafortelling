...
query = """
    ...
    from `pia_bigquery_sink_v1_dataset_prod.behovsvurdering-bigquery-v1` as bv
        left join
            (select id, endret as paabegynt from `pia_bigquery_sink_v1_dataset_prod.behovsvurdering-bigquery-v1` where status="PÅBEGYNT") as bv_paabegynt
        on bv_paabegynt.id = bv.id
        left join
            (select id, endret as avsluttet from `pia_bigquery_sink_v1_dataset_prod.behovsvurdering-bigquery-v1` where status="AVSLUTTET") as bv_avsluttet
            on bv_avsluttet.id = bv.id
        left join
            (select id, endret as slettet from `pia_bigquery_sink_v1_dataset_prod.behovsvurdering-bigquery-v1` where status="SLETTET") as bv_slettet
...
def statusflyt_behovsvurdering(df_bv: pd.DataFrame) -> go.Figure:
    ...
    fig.add_trace(
        go.Sankey(
            node=dict(
                pad=200,
                label=["Alle", "Opprettet", "Påbegynt", "Avsluttet", "Slettet"],
                # node position in the open interval (0, 1)
                x=[0.05, 0.25, 0.5, 0.75, 0.95],
                y=[0.5, 0.5, 0.75, 0.9, 0.1],
            ),
            link=dict(
                source=[0, 1, 2, 3, 1, 2],
                target=[1, 2, 3, 4, 4, 4],
                value=[
                    opprettet,
                    opprettet_til_paabegynt,
                    paabegynt_til_avsluttet,
