...
import pandas as pd
from datafortelling_utils.dataloader import last_inn_data_statistikk
data_statistikk = last_inn_data_statistikk(
    project=PROJECT,
    dataset=DATASET,
    resultatområde=RESULTATOMRÅDE,
    limit=" WHERE tidsstempel < '2026-04-01' ",
)

# Hent data om statusendringer for saker
from datafortelling_utils.datahandler import split_data_statistikk
data_status, _, _ = split_data_statistikk(data_statistikk)
```

```{python}
# Hent data om ikke aktuelle saker
from datafortelling_utils.datahandler import explode_ikke_aktuell_begrunnelse
ikke_aktuell = explode_ikke_aktuell_begrunnelse(data_status)
```

::: {.callout-important}
## Saksflyt blir ikke lenger oppdatert
NB! Denne siden blir ikke lenger oppdatert med nye tall. Siste data på denne siden er fra 2026-03-31.
:::
...
```{python}
from datafortelling_utils.statistikk import statusflyt
...
### Nåsituasjon
```{python}
from datafortelling_utils.statistikk import antall_saker_per_status
...
```{python}
from datafortelling_utils.statistikk import saker_per_status_per_måned
...
```{python}
from datafortelling_utils.statistikk import gjennomstrømmingstall
...
Her vises profilen til virksomheter som er registrert i Fia. Grafene viser antall saker per antall arbeidsforhold, sykefravæarsprosent, sektor og de 10 vanligste hovednæringer.
...
```{python}
from datafortelling_utils.statistikk import virksomhetsprofil
...
```{python}
from datafortelling_utils.helper import ikke_aktuell_begrunnelse_sortering
from datafortelling_utils.konstanter import ikkeaktuell_hovedgrunn
...
```{python}
from datafortelling_utils.leveranse import begrunnelse_ikke_aktuell
