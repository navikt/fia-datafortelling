...
def dager_siden_siste_oppdatering(
    ...
    siste_oppdatering = beregn_siste_oppdatering(data_status, data_eierskap, data_leveranse)

    fig = go.Figure(data=[go.Histogram(x=siste_oppdatering["dager_siden_siste_oppdatering"], nbinsx=20)])
    fig.update_layout(
        height=500,
        width=850,
...
def urørt_saker_over_tid(
    ...
    data = data.sort_values(["saksnummer", "endretTidspunkt"], ascending=True).reset_index()
    ...
    data.loc[data.neste_endretTidspunkt.astype(str) == "NaT", "neste_endretTidspunkt"] = now
    ...
    urørt_per_status_og_dato = dict(zip(aktive_statuser, [[0] * len(alle_datoer)] * len(aktive_statuser)))

    for _, row in data[data.mer_enn_x_dager & data.aktiv_status].iterrows():
        status = row["status"]
        endret_tidspunkt = row["endretTidspunkt"]
        neste_endret_tidspunkt = row["neste_endretTidspunkt"]

        urørt_intervall = []
        for dato in alle_datoer:
            if (dato > endret_tidspunkt + timedelta(days=antall_dager)) & (dato < neste_endret_tidspunkt):
                urørt_intervall.append(1)
            else:
                urørt_intervall.append(0)
        urørt_per_status_og_dato[status] = [sum(x) for x in zip(urørt_per_status_og_dato[status], urørt_intervall)]
    ...
    fig.update_layout(
        height=600,
        xaxis={"autorange": True, "rangeslider": {"autorange": True, "visible": True}},
    )
