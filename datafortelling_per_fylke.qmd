...
```{python}
alle_iatjenester_og_status = iatjeneste_og_status_sortering(data_leveranse)
```
...
```{python}
antall_leveranser_per_tjeneste(data_siste_leveranse_fylke, alle_iatjenester_og_status)
```
...
## Ikke aktuell
Saker som ble satt til "Ikke aktuell" i det siste 365 dagene.\

```{python}
data_input = data_status_fylke[data_status_fylke.siste_status=="IKKE_AKTUELL"].drop_duplicates("saksnummer", keep="last")
data_input = get_data_siste_x_dager(data_input)
virksomhetsprofil(data_input)
```

## Fullført
Saker som ble satt til "Fullført" i det siste 365 dagene.\
