---
title: "Fia datafortelling"
execute:
    echo: false
    warning: false
format:
    html:
        toc: true
        toc-title: Indeks
        page-layout: full
        embed-resources: true
jupyter: python3
---

Statistikk som viser bruk av Fagsystemet for IA (Fia).

<span style="color:red;">
NB! Dette er ikke offisiell statistikk og må ikke deles utenfor NAV.
</span>

Ansvarlig team er [Team Pia](https://teamkatalog.nav.no/team/b347ac99-c382-4e5a-a1dd-532e91110e2a).
 Kontakt oss på [slack](https://nav-it.slack.com/archives/C02DL347ZT2) eller på e-post `pia` ved `nav.no`. 


```{python}
import os
from datetime import datetime
import pytz

from code.helper import (
    load_data,
    load_data_deduplicate,
    preprocess_data_statistikk,
    preprocess_data_leveranse,
)
from code.plots import (
    aktive_saker_per_fylke,
    dager_siden_siste_oppdatering,
    antall_saker_per_status,
    antall_leveranser_per_sak,
    antall_leveranser_per_tjeneste,
    antall_leveranser_per_modul,
    virksomhetsprofil,
    statusflyt,
)
from code.tables import moduler_per_maaned

PROJECT = os.environ['GCP_PROJECT']
DATASET = os.environ['DATASET']
data_statistikk = load_data_deduplicate(PROJECT, DATASET, "ia-sak-statistikk-v1")
data_leveranse = load_data(PROJECT, DATASET, "ia-sak-leveranse-v1")

data_statistikk = preprocess_data_statistikk(data_statistikk)
data_leveranse = preprocess_data_leveranse(data_leveranse)
```

## Antall saker

```{python}
print("Totalt antall saker: ", data_statistikk.saksnummer.nunique())
print("Antall aktive saker: ", data_statistikk[data_statistikk.aktiv_sak].saksnummer.nunique())
```

::: {.callout-note}

## Forklaring av begrepet "sak" i Fia

En sak i Fia er en tidsbegrenset vurdering, med eventuell bistand, av en virksomhet i IA-samarbeidet.

En aktiv sak i Fia er en sak som står i status Vurderes, Kontaktes, Kartlegges eller Vi bistår.

En sak er ansett som Avsluttet når den har fått status Ikke aktuell eller Fullført.
:::

## Saksgang i Fia

::: {.panel-tabset}
## Forenklet saksgang
![forenklet-saksgang](bilder/forenklet-saksgang.png)

## Komplett saksgang
![komplett-saksgang](bilder/komplett-saksgang.png)

:::

Grafen nedenfor viser antall statusendringer som var registrert i Fia siden starten 10.05.2022.
Vi beholder status Ny her for å kunne vise hvor mange som står i Vurderes nå.
```{python}
statusflyt(data_statistikk)
```


## Antall saker per status
Her vises antall saker per sine nåværende status, bortsett fra saker i status Slettet og Ny.

```{python}
antall_saker_per_status(data_statistikk[~data_statistikk.siste_status.isin(["SLETTET", "NY"])])
```
{{< include includes/status-forklaring.qmd >}}


## Antall aktive saker per fylke

```{python}
aktive_saker_per_fylke(data_statistikk)
```

## Dager siden siste oppdatering i Fia
Grafene under viser hvor mange dager det har gått siden siste oppdatering per sak i Fia.
Vi teller som oppdatering endringer av status, leveranser som blir lagt til eller fullført, og tatt eierskap.
Hvis en statusendring blir angret (ved bruk av tilbake-knappen) eller en leveranse blir slettet, telles ikke den hendelsen og den respektive slettet/angret hendelsen som oppdateringer.
Vi viser disse tallene delt på status, siden statusene kan ha ulike typiske varigheter.
Grafene gir en indikasjon på hvor mange saker ligger urørt i Fia.

::: {.panel-tabset}
## Alle aktive saker
```{python}
dager_siden_siste_oppdatering(data_statistikk, data_leveranse)
```
## Vurderes
```{python}
dager_siden_siste_oppdatering(
    data_statistikk[data_statistikk.siste_status=="VURDERES"],
    data_leveranse
)
```
## Kontaktes
```{python}
dager_siden_siste_oppdatering(
    data_statistikk[data_statistikk.siste_status=="KONTAKTES"],
    data_leveranse
)
```
## Kartlegges
```{python}
dager_siden_siste_oppdatering(
    data_statistikk[data_statistikk.siste_status=="KARTLEGGES"],
    data_leveranse
)
```
## Vi bistår
```{python}
dager_siden_siste_oppdatering(
    data_statistikk[data_statistikk.siste_status=="VI_BISTÅR"],
    data_leveranse
)
```
:::


## Leveranser
Når en sak har status Vi bistår kan sakseier registrere hvilke IA-tjenester/moduler som er under arbeid eller levert.
En sak kan ikke gå videre til status Fullført før alle registrerte leveranser er satt som levert/fullført eller er slettet.

::: {.panel-tabset}

## Per tjeneste
```{python}
antall_leveranser_per_tjeneste(data_leveranse)
```

## Per modul
```{python}
antall_leveranser_per_modul(data_leveranse)
```

## Per sak
Grafen nedenfor viser hvor mange moduler som leveres til virksomheter under bistand.
Her gjelder alle saker som hadde eller har status Vi bistår og har registrerte leveranser.

```{python}
antall_leveranser_per_sak(data_leveranse)
```

## Per måned
Tabellen viser antall saker per modul og fullført måned, i.e., måneden som modulen ble registrert som levert/fullført.

```{python}
moduler_per_maaned(data_leveranse)
```

:::


## Virksomhetsprofil
Her vises profilene av virksomheter vi jobber med nå (Aktive saker), alle virksomheter vi har vurdert og registrert i Fia (Vurderte), og virksomheter som vi vurderte som ikke aktuell (Vurderte ikke aktuell).
Grafene viser antall unike virksomheter per antall ansatte, sykefravæarsprosent, sektor, bransjeprogram og de 10 vanligste hovednæringer.
Alle tallene er basert på informasjon vi hadde da de relevante statusendringene skjedde.
Gjeldende tidspunkter er nevnt over hver graf.

::: {.panel-tabset}

## Aktive saker
Saker med nåværende status Vurderes, Kontaktes, Kartlegges, eller Vi bistår.\
Tidspunkt: siste statusendring.
```{python}
data_input = data_statistikk[data_statistikk.aktiv_sak]
virksomhetsprofil(data_input)
```

## Vurderte
Saker som hadde eller har status Vurderes.\
Tidspunkt: statusendring fra Ny til Vurderes.
```{python}
data_input = data_statistikk[data_statistikk.status=="VURDERES"].drop_duplicates("saksnummer", keep="last")
virksomhetsprofil(data_input)
```

## Vurderte ikke aktuell
Saker som gikk fra status Vurderes til Ikke aktuell.\
Tidspunkt: statusendring fra Vurderes til Ikke aktuell.
```{python}
data_input = data_statistikk[
    (data_statistikk.status=="IKKE_AKTUELL") &
    (data_statistikk.forrige_status=="VURDERES")
].drop_duplicates("saksnummer", keep="last")
virksomhetsprofil(data_input)
```
:::

---

```{python}
print("Denne siden ble generert {}".format(datetime.now(pytz.timezone("Europe/Oslo")).isoformat()))
```
