...
from code.helper import load_data, preprocess_data
...
data_statistikk = preprocess_data(data_statistikk)
```
...
```{python}
fig = antall_saker_per_status(data_statistikk[data_statistikk.siste_status!="SLETTET"])
fig.show()
```
...
## Per tjeneste
```{python}
fig = antall_leveranser_per_tjeneste(data_leveranse)
fig.show()
```

## Per modul
```{python}
fig = antall_leveranser_per_modul(data_leveranse)
fig.show()
```

## Per sak
```{python}
fig = antall_leveranser_per_sak(data_leveranse)
fig.show()
```
...
fig = virksomhetsprofil(data_input, title="Virksomhetsprofil av alle aktive saker")
fig.show()
```
...
fig = virksomhetsprofil(data_input, title="Vurderte virksomheter")
fig.show()
```
...
fig = virksomhetsprofil(data_input, title="Virksomheter vurderte ikke aktuell")
fig.show()
```
:::
...
# Statusflyt
```{python}
fig = statusflyt(data_statistikk)
fig.show()
```
