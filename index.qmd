...
data_siste_leveranse_siste_år = data_siste_leveranse[data_siste_leveranse.saksnummer.isin(data_status_siste_år.saksnummer.unique())]
```
...
## Per tjeneste
```{python}
antall_leveranser_per_tjeneste(data_siste_leveranse_siste_år)
```

## Per modul
```{python}
modul_sortering = modul_sortering(data_siste_leveranse_siste_år)
```
::: {.panel-tabset}
## Levert
```{python}
antall_leveranser_per_modul(
    data_siste_leveranse_siste_år[data_siste_leveranse_siste_år.status=="LEVERT"], modul_sortering
)
```
## Under arbeid
```{python}
antall_leveranser_per_modul(
    data_siste_leveranse_siste_år[data_siste_leveranse_siste_år.status == "UNDER_ARBEID"], modul_sortering
)
```
## Levert og under arbeid
```{python}
antall_leveranser_per_modul(data_siste_leveranse_siste_år, modul_sortering)
```
:::
...
```{python}
leveranse_tjeneste_per_maaned(data_siste_leveranse_siste_år)
```

## Detaljer
Tabellen viser antall saker per modul og fullført måned, i.e., måneden som modulen ble registrert som levert.
```{python}
moduler_per_maaned(data_siste_leveranse_siste_år)
```
