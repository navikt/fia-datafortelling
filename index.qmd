...
```{python}
begrunnelse_sortering = ikke_aktuell_begrunnelse_sortering(data_status)
```
...
```{python}
begrunnelse_ikke_aktuell(ikke_aktuell, begrunnelse_sortering)
```

## Fra Vurderes
```{python}
begrunnelse_ikke_aktuell(
    ikke_aktuell[ikke_aktuell.forrige_status == "VURDERES"],
...
```{python}
begrunnelse_ikke_aktuell(
    ikke_aktuell[ikke_aktuell.forrige_status == "KONTAKTES"],
    begrunnelse_sortering,
)
```
...
```{python}
begrunnelse_ikke_aktuell(
    ikke_aktuell[ikke_aktuell.forrige_status == "KARTLEGGES"],
    begrunnelse_sortering,
)
```
...
```{python}
begrunnelse_ikke_aktuell(
    ikke_aktuell[ikke_aktuell.forrige_status == "VI_BISTÅR"],
    begrunnelse_sortering,
)
```
